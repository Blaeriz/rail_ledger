...
<script>
	...
	function calculateInspectorStats() {
		...
		inspectorStats.completedToday = reportData.filter(
			(/** @type {any} */ r) => r.createdAt?.startsWith(today) || r.created_at?.startsWith(today)
		).length;

		// Failed batches (more comprehensive check)
		inspectorStats.failedBatches = batchData.filter(
			(/** @type {any} */ b) =>
		...
		const passedBatches = batchData.filter(
			(/** @type {any} */ b) => b.qc_status === 'Pass' || b.qc_status === 'PASS'
		).length;

		inspectorStats.passRate =
			totalBatches > 0 ? parseFloat(((passedBatches / totalBatches) * 100).toFixed(1)) : 0;

		// Average quality score from reports
		const scores = reportData
			.map((/** @type {any} */ r) => r.quality_score || r.overall_score || 0)
			.filter((s) => s > 0);
		inspectorStats.avgScore =
			scores.length > 0
		...
		inspectorStats.efficiency =
			inspectorStats.completedToday > 0
				? parseFloat(
						(
							(inspectorStats.completedToday /
								(inspectorStats.completedToday + inspectorStats.urgentInspections)) *
							100
						).toFixed(1)
					)
				: 0;
	...
	function calculateWeeklyPerformance() {
		...
		for (let i = 6; i >= 0; i--) {
			...
			const dayReports = reportData.filter(
				(/** @type {any} */ r) =>
...
<div class="p-6">
	<div class="mb-8">
		<h1 class="mb-2 text-3xl font-bold text-white">Quality Analysis</h1>
	...
	<div class="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
		...
		>
			<div class="flex items-center justify-between">
				<div>
					<p class="mb-2 text-sm font-medium text-gray-400">Completed Today</p>
		...
		>
			<div class="flex items-center justify-between">
				<div>
					<p class="mb-2 text-sm font-medium text-gray-400">Urgent Inspections</p>
		...
		>
			<div class="flex items-center justify-between">
				<div>
					<p class="mb-2 text-sm font-medium text-gray-400">Pass Rate</p>
