...
<script>
  ...
  let vendorChartData = { labels: /** @type {string[]} */ ([]), datasets: [{ label: 'Batches', data: /** @type {number[]} */ ([]), backgroundColor: 'rgba(139, 92, 246, 0.8)', borderColor: '#8B5CF6', borderWidth: 3, borderRadius: 8 }] };

  let monthlyTrendsData = { labels: /** @type {string[]} */ ([]), datasets: [{ label: 'Passed', data: /** @type {number[]} */ ([]), borderColor: '#10B981', backgroundColor: 'rgba(16, 185, 129, 0.15)', borderWidth: 4, fill: true, tension: 0.4 }, { label: 'Failed', data: /** @type {number[]} */ ([]), borderColor: '#EF4444', backgroundColor: 'rgba(239, 68, 68, 0.15)', borderWidth: 4, fill: true, tension: 0.4 }] };
...
<svelte:head>
  <title>Analytics Dashboard - Rail Ledger</title>
</svelte:head>
