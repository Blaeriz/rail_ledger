...
<div class="p-4 sm:p-6 lg:p-8">
  ...
  <div class="mb-6 sm:mb-8">
    <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-2">Tickets</h1>
    ...
    {#if selectedTicket}
      <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h3 class="text-lg font-semibold text-white mb-2">{selectedTicket.title}</h3>
            <p class="text-gray-400 text-sm">Ticket ID: {selectedTicket.ticket_id}</p>
          </div>
          <div class="flex flex-wrap gap-2">
            <span class="px-3 py-1 rounded-full text-xs font-semibold border-2 {getPriorityColor(selectedTicket.priority)}">
              {selectedTicket.priority}
            </span>
            <span class="px-3 py-1 rounded-full text-xs font-semibold border-2 {getTicketStatusColor(selectedTicket.status)}">
              {selectedTicket.status}
            </span>
            <span class="px-3 py-1 rounded-full text-xs font-semibold border-2 {getCategoryColor(selectedTicket.category)}">
        ...
        <div>
          ...
          <p class="text-white bg-gray-700 p-3 rounded-lg">{selectedTicket.description}</p>
        </div>

        <!-- Details Grid -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h4 class="text-sm font-medium text-gray-300 mb-2">Created By</h4>
            <p class="text-white">{selectedTicket.created_by}</p>
          </div>
          <div>
            <h4 class="text-sm font-medium text-gray-300 mb-2">Assigned To</h4>
          ...
          <div>
            <h4 class="text-sm font-medium text-gray-300 mb-2">Last Updated</h4>
        ...
        {#if selectedTicket.resolution_notes}
          <div>
            <h4 class="text-sm font-medium text-gray-300 mb-2">Resolution Notes</h4>
            <p class="text-white bg-gray-700 p-3 rounded-lg">{selectedTicket.resolution_notes}</p>
          </div>
        {/if}
        ...
        {#if selectedTicket.attachments && selectedTicket.attachments.length > 0}
          <div>
            <h4 class="text-sm font-medium text-gray-300 mb-2">Attachments</h4>
            <div class="space-y-2">
