...
<svelte:head>
  <title>AI Reports - Viewer Dashboard</title>
</svelte:head>
...
<div class="p-6 space-y-6">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
    <div>
      ...
      <p class="text-gray-400">AI-powered insights and analytics for system monitoring</p>
    </div>
    <div class="mt-4 sm:mt-0 flex flex-col sm:flex-row gap-3">
      <select
        bind:value={generateType}
        class="px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
      >
      ...
      <button 
        ...
        class="bg-green-600 hover:bg-green-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white px-6 py-3 rounded-lg font-medium transition-all duration-300 border border-green-500/30 hover:border-green-400/50 hover:shadow-lg hover:shadow-green-500/20"
      >
        {#if isGenerating}
          <div class="animate-spin rounded-full h-4 w-4 border-b-2 border-white inline mr-2"></div>
          Generating...
        {:else}
          <Icon name="chart" size="w-4 h-4" className="inline mr-2" />
          Generate AI Report
        {/if}
      </button>
    </div>
    ...
    <div class="flex-1">
      <div class="relative">
        ...
        <input
          ...
          class="w-full pl-10 pr-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
        />
      </div>
    </div>
    <div class="sm:w-48">
      <select
        ...
        class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
      >
      ...
      {#each filteredReports() as report}
        ...
        >
          <div class="flex items-start justify-between mb-4">
            <div class="flex-1">
              <h3 class="text-white font-semibold text-lg mb-2 group-hover:text-green-400 transition-colors">
                {report.title}
              </h3>
          ...
          <div class="space-y-2 mb-4">
            ...
            <div class="flex items-center text-sm text-gray-400">
              <Icon name="analytics" size="w-4 h-4" className="mr-2" />
              {report.type}
            </div>
          ...
          <div class="flex items-center text-green-400 text-sm font-medium group-hover:text-green-300 transition-colors">
            View Details
    ...
    >
      ...
      >
            ...
            <div>
              ...
              <ul class="space-y-2">
                {#each selectedReport.recommendations as recommendation}
                  <li class="flex items-start text-gray-300">
                    <Icon name="plus" size="w-5 h-5" className="text-green-400 mr-3 mt-0.5 flex-shrink-0" />
                    {recommendation}
                  </li>
                {/each}
              </ul>
        ...
        <div class="p-6 border-t border-gray-700 flex justify-end space-x-3">
          ...
          <button class="px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors">
            Export Report
          </button>
        </div>
      </div>
