	...
	function renderMap() {
		...
		svg.selectAll('.state')
			...
			.on('mouseenter', function(event, d) {
				const stateData = getStateData(d.properties.name);
				if (stateData) {
					// Get the centroid of the state for better positioning
					...
					dispatch('mouseenter', {
						...
						x: svgRect.left + centroid[0] * (svgRect.width / width),
						y: svgRect.top + centroid[1] * (svgRect.height / height)
					});
				}
				d3.select(this)
					.attr('stroke', '#ffffff')
			...
			.on('mousemove', function(event, d) {
				// Use the centroid for consistent positioning
				const centroid = path.centroid(d);
				const svgRect = mapContainer.getBoundingClientRect();
